...
namespace Drupal\uc_expresspayepos\Controller;

use Drupal\uc_expresspayepos\Plugin\Ubercart\PaymentMethod\ExpresspayEpos;
...
class ExpresspayEposController extends ControllerBase
{
	...
	public function complete()
	{
		...
		if ($plugin->getPluginId() != 'expresspayepos') {
		...
		if ($valid == false) {
			uc_order_comment_save($order->id(), 0, 'Эксперсс платежи: EPOS -> Цифровая подпись не совпала!', 'admin');
			return ['#plain_text' => "Эксперсс платежи: EPOS -> Цифровая подпись не совпала!"];
		}
		...
		$epos_code  = $configuration['eposCode'] ."-";
		$epos_code  .= "1-";
		$epos_code .= $order->id();
	...
	function validSignature($settings, $signature)
	{
		...
		$validSignature = ExpresspayEpos::compute_signature($signature_param, $token, $secret_word, 'add_invoice_return');
